...
import { Button, Spin, Empty, Tag, Tooltip, Badge, Card, Checkbox, Row, Col } from "antd";
import {
  CloseOutlined,
  FullscreenOutlined,
...
const KnowledgeGraphPanel = ({
  ...
  }, [isVisible, isCollapsed, graphData, hiddenEntityTypes, hiddenRelationshipTypes, layout]);
  ...
  }, [graphData, seedEntities, hiddenEntityTypes, hiddenRelationshipTypes, layout, isFullscreen]);
  ...
  const handleRelationshipTypeToggle = (relType) => {
    ...
    if (isHidden) {
      setHiddenRelationshipTypes(hiddenRelationshipTypes.filter((t) => t !== relType));
    } else {
    ...
    return (
      <div className="kg-filter-bar relationship-filter-bar">
        <div className="filter-bar-header">
          <FilterOutlined />
          <span>关系类型筛选</span>
          <Button type="link" size="small" onClick={handleSelectAllRelationships}>
