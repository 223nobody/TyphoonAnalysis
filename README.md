...
- 卫星云图智能分析
- 支持红外/可见光图像单图上传
- Few-shot 混合分析（Qwen-VL + OpenCV/融合）
- 基于标注样例学习台风眼位置并生成 AI 报告
- 上传预览、重复上传拦截和图像管理
...
│   │   └── services/          # 业务逻辑
│   │       ├── ai/            # AI服务
│   │       │   ├── qwen_video_service.py # 视频分析服务
│   │       │   └── qwen_image_service.py # 图像few-shot分析服务
│   │       ├── asr/           # 语音识别服务 (阿里云 NLS)
│   │       ├── crawler/       # 爬虫服务
│   │       ├── image/         # 图像处理
│   │       │   ├── image_service.py      # 图像分析主服务
│   │       │   ├── opencv_analyzer.py    # OpenCV分析器
│   │       │   ├── fusion_analyzer.py    # 融合分析器
│   │       │   └── dl_analyzer.py        # 迁移学习分析器
│   │       ├── lstm/          # LSTM预测
│   │       └── scheduler/     # 定时任务
│   ├── data/                  # 数据与图像样例
│   │   └── images/test/       # few-shot标注样例与 examples.json
│   ├── main.py                # 应用入口
│   ├── data.py                # 数据导入脚本
...
- ✅ 图片分析 Few-shot 混合模式（Qwen-VL + OpenCV/融合）
- ✅ 标注样例驱动的台风眼学习与增强 AI 报告
- ✅ 上传成功预览与更丰富的图像分析结果展示

**优化改进**:

- ✅ 同名图片唯一存储与重复上传提示
- ✅ 图像分析接口、数据模型与数据库结构同步更新
