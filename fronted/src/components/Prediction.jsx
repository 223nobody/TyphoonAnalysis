...
function Prediction() {
  ...
  const handleArbitraryPrediction = async () => {
    ...
    try {
      ...
      const response = await predictFromArbitraryStart(
        ...
        arbitraryForm.startPressure ? parseFloat(arbitraryForm.startPressure) : null,
        arbitraryForm.startWindSpeed ? parseFloat(arbitraryForm.startWindSpeed) : null,
  ...
  return (
    <div>
      ...
      {result && (
        <div style={{ marginTop: "20px" }}>
          {(result.type === "path" || result.type === "arbitrary" || result.type === "virtual") &&
            renderPredictionTable(result.data, true)}
          {result.type === "intensity" && renderPredictionTable(result.data, true)}
