  ...
  const handleIntensityPrediction = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/prediction/intensity`,
        {
  ...
  const renderPathResult = (data) => {
    ...
    return (
      <div className="info-card">
        ...
        <p>
          <strong>预测时长:</strong> {data.hours} 小时
        </p>
        <p>
          <strong>预测点数:</strong> {data.predictions.length}
        </p>
            ...
            {data.predictions.map((pred, index) => (
              <tr key={index}>
                ...
                <td style={{ textAlign: "center" }}>
                  {pred.latitude?.toFixed(2) || "N/A"}
                </td>
                <td style={{ textAlign: "center" }}>
                  {pred.longitude?.toFixed(2) || "N/A"}
                </td>
                <td style={{ textAlign: "center" }}>
                  {pred.confidence
          ...
          <tbody>
            {data.predictions.map((pred, index) => (
              <tr key={index}>
                <td>{pred.time || `+${pred.hours || index * 6}h`}</td>
                <td style={{ textAlign: "center" }}>
                  {pred.wind_speed?.toFixed(1) || "N/A"}
                </td>
                <td style={{ textAlign: "center" }}>
                  {pred.pressure?.toFixed(0) || "N/A"}
                </td>
                <td style={{ textAlign: "center" }}>
                  {pred.intensity || "N/A"}
                </td>
