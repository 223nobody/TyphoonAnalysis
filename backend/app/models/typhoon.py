...
class AskHistory(Base):
    ...
    question = Column(Text, nullable=False, comment="用户原始提问内容")
    graph_result = Column(Text, nullable=True, comment="知识图谱检索结果（GraphRAG返回的上下文）")
    graph_data = Column(JSON, nullable=True, comment="知识图谱可视化数据（nodes, relationships等）")
